...
async function loadRatingChart() {
    ...
    try {
        ...
        if (result.success) {
            ...
            renderCourseRatingSummary(result.data);
        } else {
...
function renderCourseRatingSummary(distribution) {
    const block = document.getElementById("courseRatingBlock");
    if (!block) return;

    const avgEl = block.querySelector(".font-bold");
    const countEl = block.querySelector(".typography-default");
    const starContainer = block.querySelector(".flex.flex");

    const totalVotes = Object.values(distribution).reduce((a, b) => a + b, 0);

    if (totalVotes === 0) {
        if (avgEl) avgEl.textContent = "-";
        if (countEl) countEl.textContent = "No ratings yet";
        if (starContainer) starContainer.innerHTML = renderStars(0);
        return;
    }
    ...
    const average = (sum / totalVotes).toFixed(1);

    if (avgEl) avgEl.textContent = average;
    if (countEl) countEl.textContent = `(${totalVotes} reviews)`;
    if (starContainer) starContainer.innerHTML = renderStars(parseFloat(average));
}
...
function renderStars(rating) {
    ...
    for (let i = 0; i < 5; i++) {
        if (i < fullStars) {
            stars.push(getStarSvg("text-yellow-400"));
        } else if (i === fullStars && hasHalf) {
            stars.push(getStarSvg("text-yellow-400")); // 若你有半星图标可替换此行
        } else {
            stars.push(getStarSvg("text-gray-300"));
        }
    }
...
function getStarSvg(colorClass) {
    return `
        <svg class="w-5 h-5 ${colorClass}" fill="currentColor" viewBox="0 0 20 20">
